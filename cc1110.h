...
/*
 *   Most SFRs are also accessible through XDATA address space. The register
 *   definitions for this type of access are listed below. The register names
 *   are identical to the SFR names but with the prefix X_ to denote an XDATA
 *   register. Some SFRs are not accessible through XDATA space. For clarity,
 *   entries are included for these registers. They have a prefix of _NA to
 *   denote "not available." The SFRs prefixed with _X_ are not in use, but are
 *   listed for a better overview. For register descriptions, refer to the
 *   actual SFR declarations elsewhere in this file.
 */
